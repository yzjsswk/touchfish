...
class ClipboardManager {
    ...
    private func listenToClipboardChanges() {
        DispatchQueue.global(qos: .background).asyncAfter(deadline: .now() + 0.5) { [pasteboard, listenToClipboardChanges] in
            
            func getLatestClipboardData() -> Data {
                if let strContent = pasteboard.string(forType: .string) {
//                    _ = DBManager.saveFish(id: nil, type: .text, value: strContent, source: .clipboard, tag: [])
                    return strContent.data(using: .utf8) ?? Data()
                }
                return Data()
            }
            
            let data = getLatestClipboardData()
            if self.currentData != data {
                self.updateClipboardHistory(with: ClipboardHistoryItem(data: data))
            }
