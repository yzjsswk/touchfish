...
impl MongoStorage {
    ...
    async fn create_index_if_not_exists(&self) -> YRes<()> {
        let indexes= self.collection__fish().list_index_names().await.map_err(|e| {
            err!("check index failed").trace(
                ctx!("check index -> get index on fish: self.collection__fish().list_index_names() failed", e)
            )
        })?;
        if !indexes.contains(&"idx_unique_data".to_string()) {
        ...
        if !indexes.contains(&"idx_update_time".to_string()) {
            let index = IndexModel::builder()
