...
impl<S> TopicService<S> where S: TopicStorage {
    ...
    pub fn create_topic(
        ...
        self.storage.add_topic(YUid::new(), topic_type, subject, title, extra_info)
    }
    ...
    pub fn append_message(
        ...
        self.storage.add_message(
            YUid::new(), topic.uid, level, source, title, body, has_read, extra_info,
        ).trace(
    ...
    pub fn remove_topic(&self, subject: String) -> YRes<()> {
        ...
        self.storage.remove_topic(topic.uid).trace(
    ...
    pub fn list_topic(&self) -> YRes<Vec<TopicWithMessage>> {
        ...
        let mut message_group_by_topic = messages.into_iter().fold(HashMap::new(), |mut acc, it| {
            acc.entry(it.topic_uid.clone()).or_insert(Vec::new()).push(it);
            acc
        });
        let ret = topics.into_iter().fold::<Vec<_>, _>(Vec::new(), |mut acc, it| {
            let messages = message_group_by_topic.remove(&it.uid).unwrap_or(Vec::new());
