use crate::{infra::TopicStorage, service::TopicService, Message, MessageExtraInfo, MessageLevel, Topic, TopicExtraInfo, TopicType, TopicWithMessage};
use yfunc_rust::prelude::*;

pub struct TopicFacade<S> where S: TopicStorage {
    topic_service: TopicService<S>,
}

impl<S> TopicFacade<S> where S: TopicStorage {

    pub fn new(storage: S) -> YRes<TopicFacade<S>> {
        Ok(TopicFacade {
